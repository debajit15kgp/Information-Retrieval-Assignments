...
class RANKED_RETRIEVAL_UTIL:
	...
	def load_static_score(self):
		with open("../StaticQualityScore.pkl", "rb") as fp:
...
class SCORE_UTIL:
	...
	def load_leaders(self):
		with open("../Leaders.pkl", "rb") as fp:
			self.leaders = pickle.load(fp)

	# calculate cosines similarity score
	...
	def cluster_pruning_score(self):
		try:
			...
			for doc_no in range(self.total_files):
				if(doc_no in self.leaders):
					...
					for list_leader in leaders_score.keys():
						now_list.append((self.cosine_calculate(leaders_score[list_leader],list_doc,self.denom_score[list_leader]),list_leader))
					now_list.sort(reverse=True)
			...
			now_list = []
			for list_leader in leaders_score.keys():
				now_list.append((self.cosine_calculate(leaders_score[list_leader],self.vq_idf,self.denom_score[list_leader]),list_leader))
			now_list.sort(reverse=True)
